...
Класс `GameMatrix` представляет собой матрицу, используемую для вычисления выигрышей и штрафов на основе выбранных ходов игроков.  
Взаимодействие с матрицей осуществляется через класс `MatrixReader`, который загружает данные матрицы из конфигурационных файлов.

### Игровые матрицы
В папке `config/matrices` находятся несколько вариантов игровых матриц, каждая из которых задаёт свои правила для симуляции:

- **Strict Punishment for Betrayal**  
  Предательство строго наказывается у кооператоров (-5 или -10). Кооперация приводит к большим вознаграждениям (10).

- **Balanced Trade-off**  
  Менее резкие потери у кооператоров и умеренный выигрыш у предателей.

- **Rewarding Cooperation, Deterring Betrayal**  
  Кооперация выгоднее (6 очков), а предательство приводит к постепенному снижению очков у всех участников.

- **Extreme Payoff for Solo Betrayal**  
  Одиночный предатель получает очень высокий выигрыш (20), но множественные предательства приводят к убыткам (-5).
...
#### Метастратегия
Метастратегия — это стратегия, которая использует другие стратегии для принятия решений. 
